...
function iterative_deepening(
    ...
    if best_move == Int32(-1)
        best_move = rand(validactions(board), 1)
        println("No valid moves found, returning a random move")
        show(best_move, board)
...
function minimax(
    ...
    @no_escape buffer begin
        ...
        for action_as_index in 1:idx
            action_as_index = ordered_move_buffer[action_as_index]
